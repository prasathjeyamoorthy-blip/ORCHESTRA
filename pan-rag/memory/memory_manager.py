...
class MemoryManager:
    """
    Upstash Redis-backed memory. Falls back to in-process dict when
    Upstash is not configured (dev/test).
    """
    ...
    def __init__(self):
        ...
        if (
            self._url and self._url.startswith("https://")
            and self._token and self._token != "your-token-here"
        ):
            self._mode = "upstash"
            print("[Memory] Using Upstash Redis")
        else:
            self._mode = "local"
            print("[Memory] Upstash not configured — using in-process cache (non-persistent)")
