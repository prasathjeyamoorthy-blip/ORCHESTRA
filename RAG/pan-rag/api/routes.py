...
from agent.pan_verification_upd import (
    basic_file_check,
    image_quality_check,
    run_vlm,
...
from agent.cross_validator import SessionDocumentStore

# Global session document store for cross-validation
doc_store = SessionDocumentStore()

# Driving License extraction VLM prompt
DL_PROMPT = """
...
def find_user_id_for_session(session_id: str) -> str:
    """Helper to locate user_id folder containing the session json file."""
    from agent.flow_manager import SESSIONS_DIR
    try:
...
async def upload_document(
    ...
    user_id = find_user_id_for_session(session_id)
    ...
    # ── Technical checks ──
    ok, msg = basic_file_check(file_bytes, doc_type.capitalize(), file.filename)
    if not ok:
        raise HTTPException(status_code=422, detail=msg)

    ok, msg = image_quality_check(file_bytes, doc_type.capitalize(), file.filename)
    if not ok:
        raise HTTPException(status_code=422, detail=msg)
    ...
    try:
        if doc_type_lower == "aadhaar":
            raw = run_vlm(AADHAAR_PROMPT, file_bytes, file.filename)
            data = AadhaarData(**raw)
            validation_errors = validate_aadhaar_fields(data)
            ...
            if user_id and user_id != "anonymous":
                ...
                person_name = extracted.get("name") or "Unknown"
                if mobile_number:
                    from agent.supa import get_or_create_person, save_document as supa_save_document, delete_old_documents
                    person_id = get_or_create_person(user_id, mobile_number, person_name)
                    try:
                        delete_old_documents(person_id)
                    except Exception:
                        pass
                    doc_id = supa_save_document("aadhaar", extracted, user_id, person_id)
            ...
            extraction_result = {
                ...
                'verified': len(validation_errors) == 0,
                'message': (
                    '✅ Aadhaar extracted and verified successfully'
                    if not validation_errors
                    else f'⚠️ Aadhaar extracted with {len(validation_errors)} issue(s)'
                ),
                'summary': {
        ...
        elif doc_type_lower == "photograph":
            raw = run_vlm(PHOTO_PROMPT, file_bytes, file.filename)
            data = PhotoData(**raw)
            validation_errors = validate_photo_fields(data)
            extracted = data.model_dump()
            ...
            extraction_result = {
                ...
                'message': (
                    ...
                    if not validation_errors
                    else f'⚠️ Photograph has {len(validation_errors)} issue(s)'
                ),
                'summary': {
        ...
        elif doc_type_lower == "driving_license":
            raw = run_vlm(DL_PROMPT, file_bytes, file.filename)
            validation_errors = []
            if not raw.get('name'):
                validation_errors.append('Could not extract name from Driving License')
            if raw.get('document_type', '').lower() != 'driving_license':
                validation_errors.append(f"Document does not appear to be a Driving License (detected: {raw.get('document_type')})")
            ...
            extraction_result = {
                ...
                'message': (
                    ...
                    if not validation_errors
                    else f'⚠️ Driving License extracted with {len(validation_errors)} issue(s)'
                ),
                'summary': {
        ...
        elif doc_type_lower == "signature":
            raw = run_vlm(SIGNATURE_PROMPT, file_bytes, file.filename)
            data = SignatureData(**raw)
            validation_errors = validate_signature_fields(data)
            extracted = data.model_dump()
            doc_store.save(session_id, "signature", extracted)
            ...
            extraction_result = {
                ...
                'message': (
                    ...
                    if not validation_errors
                    else f'⚠️ Signature has {len(validation_errors)} issue(s)'
                ),
                'summary': {
    ...
    if doc_store.all_uploaded(session_id, REQUIRED_DOCS):
        res = doc_store.cross_validate(session_id)
        extraction_result['cross_validation'] = {
            'complete':     True,
            'passed':       res.passed,
            'errors':       res.errors,
            ...
            'message': (
                ...
                if res.passed
                else f'❌ Cross-validation failed with {len(res.errors)} error(s). Please fix and re-upload.'
            ),
        }
        if not res.passed:
            extraction_result['status'] = 'cross_validation_failed'
            extraction_result['verified'] = False
    else:
    ...
    # ── Accompanying Message Collection ──
    if message.strip():
        if _fm.has_active_flow():
            merge_form_fields(_fm, message)
