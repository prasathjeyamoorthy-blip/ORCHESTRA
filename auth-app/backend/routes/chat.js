...
if (
  UPSTASH_URL && UPSTASH_URL.startsWith('https://') &&
  UPSTASH_TOKEN && UPSTASH_TOKEN !== 'your-token-here'
) {
  const { Redis } = require('@upstash/redis');
  redis = new Redis({ url: UPSTASH_URL, token: UPSTASH_TOKEN });
  console.log('✅ Redis (Upstash) enabled');
}

// ── Cache helpers (Redis → in-process fallback) ───────────────────────────────
const memCache = new Map();
const cacheGet    = async (k)         => redis ? redis.get(k)                    : (memCache.get(k) ?? null);
const cacheSet    = async (k, v, ttl) => redis ? redis.set(k, v, { ex: ttl })   : memCache.set(k, v);
const cacheDel    = async (k)         => redis ? redis.del(k)                    : memCache.delete(k);
// Refresh TTL on an existing key without re-fetching the value
const cacheTouch  = async (k, ttl)    => redis ? redis.expire(k, ttl)           : null;
...
async function loadAgentMemory(userId) {
  if (!redis) {
    return { history: [], summary: '', preferences: {} };
  }
...
async function triggerSummarization(userId, history, existingSummary) {
  if (!redis || history.length <= 20) return;
...
async function triggerPreferenceExtraction(userId, history, existingPreferences) {
  if (!redis || history.length % 5 !== 0) return;
...
function _isAskingAboutStoredData(message) {
  ...
  const SPECIFIC_PATTERNS = {
    name: [
      'what is my name', 'whats my name', 'my name', 'what name did i give',
      'what name i provided', 'do you know my name', 'tell me my name',
      'what did i say my name was', 'remind me my name',
    ],
    email: [
      'what is my email', 'whats my email', 'my email', 'what email did i give',
      'what email i provided', 'do you know my email', 'tell me my email',
      'my email address', 'what email address',
    ],
    phone: [
      'what is my phone', 'whats my phone', 'my phone', 'what phone did i give',
      'what phone number', 'my phone number', 'my mobile', 'my contact',
      'what number did i give', 'tell me my phone',
    ],
    pan: [
      'what is my pan', 'whats my pan', 'my pan', 'what pan did i give',
      'my pan number', 'my pan card', 'what pan number', 'do you have my pan',
      'tell me my pan', 'what pan i provided',
    ],
    aadhaar: [
      'what is my aadhaar', 'whats my aadhaar', 'my aadhaar', 'what aadhaar did i give',
      'my aadhaar number', 'what aadhaar number', 'do you have my aadhaar',
    ],
    address: [
      'what is my address', 'whats my address', 'my address', 'what address did i give',
      'where do i live', 'my location', 'my city', 'tell me my address',
    ],
    income: [
      'what is my income', 'whats my income', 'my income', 'my salary',
      'what salary did i give', 'what income did i give', 'how much do i earn',
      'tell me my income', 'tell me my salary',
    ],
    dob: [
      'what is my dob', 'whats my dob', 'my date of birth', 'my birthday',
      'when was i born', 'my birth date', 'what dob did i give',
    ],
    mother_name: [
      'what is my mother name', 'whats my mother name', 'my mother name',
      'my mothers name', "my mother's name", 'what mother name did i give',
    ],
  };
...
router.get('/history/:sessionId', verifyToken, async (req, res) => {
  try {
    const { history } = await loadHistory(req.user.id, req.params.sessionId);
